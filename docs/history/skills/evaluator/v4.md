...
Write a distinct immutable private result for every verification attempt,
including attempt identity, implementation identity, evaluator revision,
classifications, mandatory/regression results, diagnostics, and
evaluator-integrity status. Maintain a private attempt ledger linking every
result to the exact implementation and evaluator revision. Never overwrite an
