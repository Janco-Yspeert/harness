...
support file. This non-circular identity is canonical for revision comparison.
...
5. For every eligible evaluator revision used by an attempt, establish its
   ...
   directory order. Copy the exact frozen artifact bundle: specification, all
   hidden tests, case manifest, and all support files. The separately preserved
