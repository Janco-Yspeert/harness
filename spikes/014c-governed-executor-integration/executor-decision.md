...
| Pinned instructions        | NE                                                                                                                                                                                                                                                                                                  | **Pass (static).** The exact host-delivered skill and contract bytes are passed as `--system-prompt`. `--setting-sources ""`, `--strict-mcp-config`, `--disable-slash-commands` and `--restricted` exclude ambient CLAUDE.md, settings, plugins, hooks, skills and MCP servers. Governed launches omit `--safe-mode` because it also disables the explicitly passed Harness MCP server (see "Live smoke corrections"). The assignment is verified against the Role Grant identities before launch.                                                                                  |
...
| Structured transport       | NE                                                                                                                                                                                                                                                                                                  | **Pass (static).** `--output-format stream-json --verbose` gives typed events. Worker operations are a repository-owned stdio MCP server passed with `--mcp-config`. Live smoke showed that `--safe-mode` does not admit an explicitly passed `--mcp-config` server, so governed launches no longer use it. The adapter reports `assignment-not-delivered`, with the reported server status, when `init.mcp_servers` does not show `harness` connected.                                                  |
...
## Live smoke corrections

Three governed live smoke runs (`live-smoke/`, committed at `bfd14a5`) used
Claude Code `2.1.280` and `codex-cli 0.155.1`. None produced a semantic
result. The public records show two separate defects:

- **Claude.** `init` reported no connected `harness` server, and no Harness
  tool call followed. Spike 013a had already recorded that safe mode excludes
  MCP (`spikes/013a-Workflow-execution-friction/implementation-report.md`).
  Governed launches now use `GOVERNED_CLAUDE_FLAGS`: the protected flags
  without `--safe-mode`. The legacy protected mode is unchanged. The
  diagnostic now names the reported server status (`absent`, `pending`,
  `failed`, and so on).
- **Codex.** The run exited cleanly without calling `submitResult` and without
  any command denial. Under `approval_policy="never"`, Codex declines MCP tool
  calls that need approval. The adapter now pre-approves only the Harness
  server's tools with
  `mcp_servers.harness.default_tools_approval_mode="approve"`. This value is
  a variant of the `AppToolApproval` setting present in the installed Codex
  binary. It widens no sandbox, network or command permission, because every
  Harness call is still authenticated, execution-bound and host-validated.
