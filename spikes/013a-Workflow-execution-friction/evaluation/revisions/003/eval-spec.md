...
- **E2** `canonical-authority-adoption.test.ts` - two independent fixtures
  exercising AC16/AC17/AC19's general "fresh runner resumes from already-valid
  canonical workflow authority" requirement at two different checkpoints
  (spike.md AC16; Design Map "Shared contracts" states this generally, with no
  restriction to a particular checkpoint). Fixture 1 (revision 001): freezes
  canonical `brief-frozen`/`design-map-frozen` authority in a fixture,
  initializes a fresh `.workflow` state with no local history for those
  phases, and asserts the runner derives `evaluator-prepare` (not an earlier
  or later phase) as next, and does not fabricate local dispatch/completion
  records for the adopted phases. Fixture 2 (added in revision 003, repairing
  a coverage gap fixture 1 alone left - see that revision's private repair
  record): freezes canonical authority through `implementation-handoff` in a
  second fixture, initializes a fresh `.workflow` state with no local
  dispatch/completion history for any phase (including `implementation`), and
  asserts the runner derives `evaluator-verify` (not `implementation` again,
  not skipped ahead to `as-built`) directly from that canonical authority
  alone, and does not fabricate a local `implementation` dispatch/completion
  record. Covers AC16, AC17, AC19; R16, R17, R19; N4; I4. Executable; fixture 1
  currently PASSES against the candidate that produced Spike 013a cycle 001's
  promoted PASS, fixture 2 currently FAILS against that same candidate for the
  exact reason human acceptance identified (implementation-handoff canonical
  authority is not consulted when deriving evaluator-verify eligibility); both
  fixtures FAIL against the pre-implementation baseline.
...
- Revision 003 (current): standalone `repair`-mode coverage correction of E2
  (`.hidden-test/canonical-authority-adoption.test.ts`), authorized by Spike
  013a cycle 001's closed human rejection (`IMPLEMENTATION_GAP`, secondary
  `EVALUATOR_COVERAGE_DEFECT`). Revision 001/002's E2 exercised only the
  earliest instance of AC16/AC17/AC19's general canonical-authority-adoption
  requirement (`brief-frozen`/`design-map-frozen` -> `evaluator-prepare`); it
  did not exercise a later instance of that same general requirement and
  therefore could not falsify the later-checkpoint-only implementation human
  acceptance identified (canonical `implementation-handoff` authority not
  consulted when deriving `evaluator-verify` eligibility). This revision adds
  a second, independent fixture exercising the same general requirement one
  checkpoint later (`implementation-handoff` -> `evaluator-verify`), using the
  same already-frozen mechanism (canonical `authority record` + a fresh
  `.workflow init` + `dispatch`) fixture 1 already relies on. AC16/AC17/AC19's
  requirements, decision rules, and every other case are unchanged. Full
  details, traceability, and the acceptance-semantics-preserved attestation
  are in `.eval/revisions/003/repair-record.md`. Revision 002 (source) is
  archived at `.eval/revisions/002/`.
