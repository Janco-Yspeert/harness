...
// (session-stop.test.ts), renumbered to this spec's E-ids. Default
// construction path only (I11).

test("E15: stopping a session returns 204 only once cleanup is complete and the slot is reusable", async () => {
...
test("E16: stopping a session while attached closes the client connection", async () => {
  const host = await startHarnessHost(0);
  try {
    const id = await createSession(host.url);
    const socket = await connect(host.url, id);
...
test("E17: stopping a never-issued session id returns 404", async () => {
  const host = await startHarnessHost(0);
  try {
...
test("E18: stopping an already-stopped session id returns 404, not 204", async () => {
  const host = await startHarnessHost(0);
  try {
    const id = await createSession(host.url);
...
test("E19: stopping the wrong id while a session is active returns 404 and leaves the active session running", async () => {
  const host = await startHarnessHost(0);
  try {
    const id = await createSession(host.url);
    ...
    // Confirm the active session is still attachable (not disturbed by the
    // failed delete).
    const socket = await connect(host.url, id);
    await disconnect(socket);
  } finally {
