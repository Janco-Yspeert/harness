...
- **E24** — Purpose: failure to finalize even through bounded fallback uses the
  existing deletion failure path. Verifies: R18, N10. Preconditions: active turn
  in progress, small grace period; the peer's `AppServerProcess.kill()` is
  intercepted so no signal is ever actually delivered to the underlying process
  (`makeUnkillableAppServerProcess` — see Limitations for why the v1 design,
  having the peer merely ignore `SIGTERM`, was replaced: any implementation that
  correctly escalates to `SIGKILL` defeats that regardless of correctness, since
  `SIGKILL` cannot be ignored by any process). Action: `DELETE /sessions/:id`.
  Expected: response is not `204` and is `>= 500` (existing failure path),
  observed within an evaluator-chosen, disclosed bound (10s) — chosen because
  spike.md specifies no exact number for this step, only that Harness "must not
  wait indefinitely" (N10); the bound distinguishes "never resolves" from
  "resolves after bounded fallback genuinely fails." Mandatory: yes.
...
| N10 | E21, E22, E24                   | yes                                   | codex-stop.test.ts                                     |
...
- E24 (fallback finalization itself fails) is simulated by
  `makeUnkillableAppServerProcess`, which intercepts `AppServerProcess.kill()`
  so no signal is ever actually delivered to the underlying real process —
  models a process genuinely stuck in an uninterruptible OS wait state, which
  even `SIGKILL` cannot pre-empt. (v2: replaces the v1 design, which had the
  peer merely ignore `SIGTERM`; that design was defeated by any implementation
  correctly escalating to `SIGKILL`, since no process can ignore `SIGKILL`, and
  so never actually exercised this path — see Revision History.) The hidden-test
  harness reaps the real underlying process directly
  (`peer.child.kill("SIGKILL")`, bypassing the wrapper) during cleanup, ordered
  _before_ `host.close()` so cleanup itself cannot hang behind the same stuck
  finalization the case is testing. The case's own assertion is bounded by an
  explicit, disclosed 10-second `Promise.race` inside the test itself (not
  solely `node --test`'s wall-clock `--test-timeout`), so a genuine hang
  produces a precise, immediate assertion failure rather than a blunt harness
  timeout.
...
- v2 (frozen) — deliberate correction, applied 2026-08-14 after the first
  `verify` pass against implementation commit
  `38b581572fa27b0b63a0732c4063790e3d4ec320` produced 14 failing cases out
  of 31. Investigation (full account, including every diagnostic probe, in
  `<project>-hidden/<spike>/eval-result.md` from that pass, preserved at
  `<project>-hidden/<spike>/verify-2026-08-14-diagnostics/eval-result-2026-08-14-FAIL.md`)
  found 13 were evaluator defects, not implementation defects — none of which
  weaken or remove any mandatory coverage, only fix mechanical bugs in the
  frozen v1 test code:
  1. `assert.equal(x, { ...objectLiteral })` under `node:assert/strict` performs
     reference equality on objects, not deep equality, so it always fails even
     when `x` is exactly the expected value (E8, E11, E12, E14, E17, E18).
  2. A bare `socket.close()` with no `await`, racing a subsequent reattach
     `connect()` against the close handshake actually completing server-side
     (E18).
  3. `startAttachedCodexSession`'s `createBackend` closure is wired to a single
     captured peer for the host's lifetime; a same-host retry
     (`createSession(host.url)`/a second `postJson`) tried to reuse that
     already-exited peer, whose `initialize` request could never be answered —
     deadlock (E19, E21, E22).
  4. `await controller.attempt(n)` written _before_ the request that would
     trigger the Nth factory invocation — deadlocked on a promise that cannot
     resolve until the very code blocked on it runs, including for `attempt(1)`,
     not only the `attempt(2)` retry (E25, E26, E27).
  5. The `initialize` request consumed via a bare `waitForRequest()` call and
     never responded to, then `autoHandshake()` separately waiting for a
     _second_ `initialize` request that can never arrive (E3).

  A 14th case, E24, had a fundamentally unfalsifiable oracle rather than a
  simple bug: it simulated "backend cannot finalize" by having the peer ignore
  `SIGTERM` only, which any implementation that correctly escalates to `SIGKILL`
  — the normal, correct way to build this — defeats regardless of whether
  bounded-fallback failure is handled at all, since `SIGKILL` cannot be ignored
  by any process. Redesigned using `makeUnkillableAppServerProcess` (intercepts
  `kill()` itself, independent of the child process's cooperation) plus an
  explicit, disclosed 10-second bound inside the test (see E24's case text and
  Limitations). Against implementation commit `38b5815`, this corrected oracle
  found a genuine, confirmed `IMPLEMENTATION_FAILURE`:
  `CodexBackend#terminateProcess()` had no bound on the post-`SIGKILL` wait,
  violating N10. That finding — the sole reason the first `verify` pass resulted
  in `FAIL` — is the one part of this revision that reflects a real product
  defect, not an evaluator defect; it is documented for the historical record,
  not "fixed away."

  All fixes applied and self-checked (11/11 support self-checks pass, including
  two new/revised checks: a positive control confirming the dynamic-import seam
  resolves through the real module now that it exists, and a positive+negative
  control for `makeUnkillableAppServerProcess` itself — `kill()` reports success
  but the wrapped process never exits through it, unlike the v1 `ignoreSigterm`
  design). Full corrected suite
  (`<project>-hidden/<spike>/verify-2026-08-14-diagnostics/corrected-hidden-test/`
  at diagnosis time, now promoted to replace `.hidden-test/**` verbatim, with
  only relative-import-depth adjusted back for the shallower promoted location)
  typechecks cleanly and, against implementation commit `38b5815`, passes 41/42
  — the sole failure being the confirmed genuine E24/N10 finding above, failing
  cleanly via its own explicit bound in ~10s, not a harness timeout. This
  revision does not add, remove, or weaken any mandatory requirement, invariant,
  or negative requirement — R1–R24, I1–I10, and N1–N13 are unchanged from v1;
  only E24's manifest entry gained an N10 cross-reference already implied by its
  (corrected) design. Diagnostic provenance (every probe script used to isolate
  each root cause before committing to a fix) preserved at
  `<project>-hidden/<spike>/verify-2026-08-14-diagnostics/probes/`.
