...
/**
 * DIAGNOSTIC/EVALUATOR-REPAIR ADDITION (see verify-2026-08-14-diagnostics/README.md;
 * not part of the frozen eval-spec.md v1 evaluation).
 *
 * Wraps `peer.appServerProcess` so that `kill()` is accepted (returns `true`,
 * as a real `kill()` would) but silently never forwarded to the underlying
 * process — the real process is never signaled and so never emits `exit`
 * through this wrapper, regardless of which signal the implementation sends.
 *
 * This exists to give E24 (bounded-fallback-itself-fails) a genuinely
 * falsifiable oracle. The frozen v1 E24 instead had the peer ignore SIGTERM
 * specifically (`ignoreSigterm()`), which cannot distinguish a correct
 * implementation from an incorrect one: any implementation that escalates to
 * SIGKILL after a SIGTERM timeout — the normal, correct way to build this —
 * will always eventually succeed, because SIGKILL cannot be intercepted or
 * ignored by any process. That made the old E24 pass for every reasonable
 * implementation regardless of whether bounded-fallback failure was actually
 * handled correctly. This wrapper does not rely on the child process
 * cooperating with any signal at all, so it cannot be defeated by correct
 * signal escalation — it models a process stuck in an uninterruptible OS wait
 * state (e.g. blocked on I/O), which even SIGKILL cannot pre-empt.
 *
 * The real underlying process is untouched and must be reaped directly via
 * `peer.child.kill("SIGKILL")` (bypassing this wrapper) during test cleanup.
 */
export function makeUnkillableAppServerProcess(
  peer: FakeAppServer,
): AppServerProcess {
  const real = peer.appServerProcess;
  return {
    get stdin() {
      return real.stdin;
    },
    ...
    on(
      ...
      return real.on(event, listener);
    },
    kill(_signal?: NodeJS.Signals | number) {
      return true; // accepted, but never actually delivered to the real process
    },
    // The getters above always exist as own properties (unlike a true
    // optional key), which exactOptionalPropertyTypes treats as narrower
    // than AppServerProcess's `stderr?`/`pid?` — structurally equivalent at
    // runtime, so a cast is used here the same way FakeAppServer.appServerProcess does.
