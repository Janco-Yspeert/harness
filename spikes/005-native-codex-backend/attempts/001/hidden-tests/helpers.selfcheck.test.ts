...
test("selfcheck: createCodexBackend fails cleanly and informatively before src/codex-backend.ts exists (expected pre-implementation state)", async () => {
  await assert.rejects(
    () => createCodexBackend({ cwd: "/tmp" }),
    (error: unknown) => {
      const message = String(
        (error as { message?: unknown })?.message ?? error,
      );
      // Node's ESM loader reports a missing module via ERR_MODULE_NOT_FOUND;
      // this assertion documents that the dynamic-import seam itself works
      // (it reaches real module resolution) and fails for the *expected*
      // reason (module absent), not some other evaluator-harness defect.
      return /ERR_MODULE_NOT_FOUND|Cannot find module|no such file/i.test(
        message,
      );
    },
  );
