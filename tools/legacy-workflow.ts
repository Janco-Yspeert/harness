...
import { validatePreparedMap } from "../src/methodologies/harness-public.ts";
import { readLedger } from "../src/kernel/ledger.ts";
...
function authorityEvents(target: Target): AuthorityEvent[] {
  return readLedger(authorityPath(target)) as AuthorityEvent[];
}
...
export async function legacyWorkflow(args: string[]): Promise<void> {
...
if (import.meta.main)
  void legacyWorkflow(process.argv.slice(2)).catch((error: unknown) => {
